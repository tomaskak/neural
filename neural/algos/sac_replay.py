...
def sac_replay(
    ...
    for item in iter(new_data_q.get, "STOP"):
        cmd, args = item
        if cmd == "EXP" or cmd == "TEST":
            if cmd == "EXP":
                new_data = args
                replay_buf.push(new_data)

            #print(f"q_size={push_batch_q.qsize()}")
            if (
                (len(replay_buf) >= hypers["minibatch_size"] * 2
                and push_batch_q.qsize() < 200) or
                    cmd == "TEST"
            ):
                for _ in range(200 if cmd=="EXP" else 10000):
                    if push_batch_q.qsize() >= 10000:
                        break
                    with timer("push-new-batch" if cmd == "EXP" else "test-batch"):
